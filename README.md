...
> **Note on Project Status:** This repository contains the primary codebase built during the CareCloud AI Engineer Technical Assessment. Due to external vendor verification/regulatory restrictions on the Twilio trial account that prevented inbound call routing during live testing, the telephony orchestration layer was pivoted to complete the live demonstration. This repository serves as complete proof of architecture, prompt engineering, tool definitions, and backend implementation for the original Twilio + Gemini setup.
...
3. **Validation & Deployment:** FastAPI with Pydantic schemas enforces strict server-side demographic constraints before persisting data to SQLite, deployed live on Render.

---

## ✨ Features Implemented

- 🟢 **Natural Voice Interaction:** Dynamic conversational intake collecting standard U.S. patient demographic fields.
- 🟢 **Server-Side Validation:** Enforces valid 10-digit U.S. phone numbers, 2-letter state abbreviations, valid ZIP codes, and prevents future dates of birth.
- 🟢 **Function Calling / Tools:** Native Gemini tool definitions for checking caller existence and saving confirmed records.
- 🟢 **Recap & Confirmation:** Explicitly reads back all collected fields for verbal confirmation before saving.
- 🟢 **RESTful API & CRUD:** Full `/patients` REST API supporting retrieval, filtering, creation, updating, and soft deletion (`deleted_at`).
